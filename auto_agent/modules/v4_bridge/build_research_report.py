...
def _parse_frontmatter(text: str) -> dict[str, Any]:
    """YAML-lite frontmatter 파싱.

    지원 형식:
      key: value                    → {key: "value"}
      key: [a, b]                   → {key: ["a","b"]}   (flow list)
      key:                          → {key: ["a","b"]}   (block list)
        - a
        - b
    """
    ...
    while i < len(lines):
        ...
        if not line.strip() or line.lstrip().startswith("#"):
            i += 1
            continue
        # top-level key: ... 라인 (들여쓰기 없음, "-" 시작 아님)
        if ":" in line and not line.startswith((" ", "\t", "-")):
            key, _, val = line.partition(":")
            key = key.strip()
            val = val.strip()
            if val == "":
                # block list: 다음 들여쓰기 "- item" 라인 수집
                items: list[str] = []
                j = i + 1
                while j < len(lines):
                    nxt = lines[j]
                    stripped = nxt.lstrip()
                    if stripped.startswith("-") and (nxt.startswith(" ") or nxt.startswith("\t")):
    ...
    *,
    ...
    if reports_files:
        ...
        for md_path in reports_files:
            ...
            # 출처 추출 — body 인라인 [title](url) + frontmatter sources: [list]
                ...
                for url in fm_sources:
                    ...
                    if url and url not in seen_urls and url.startswith(("http://", "https://")):
                        ...
                        all_sources.append({
                            "title": url,
                            "url": url,
                            "quality_grade": _infer_quality_grade(url, str(status or "")),
                        })
