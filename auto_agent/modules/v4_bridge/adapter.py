...
from .build_targeted_claims import build_targeted_claims
...
def run_adapter(
    ...
    # ── 4b. targeted_claims.json 빌드 (v3 step_2/step_2_data/step_2b 입력) ──
    targeted_claims = build_targeted_claims(targeted_dir)
    ...
    files: dict[str, Any] = {
        ...
        "targeted_claims.json": targeted_claims,
    ...
    # ── 7. sentinel — v4-bridge 원작 프로젝트임을 표시 ──
    # runner.py 가 이 파일을 감지하면 step_2b/2c (fact-verifier/fact-fixer) 자동 스킵.
    # 이유: v4 워크플로에서 PD가 fact-check + proofread 스킬로 draft 단계에서
    # 이미 검증·교정 완료 → 다운스트림 재검증은 중복이며 노이즈 위험.
    sentinel_path = project_dir / ".v4_bridge_origin"
    sentinel_path.write_text(
        "# v4-bridge adapter sentinel\n"
        "# Generated by auto_agent.modules.v4_bridge.adapter\n"
        "# Tells runner.py to skip step_2b/2c (fact-check/proofread already done upstream).\n",
        encoding="utf-8",
    )
    artifacts.append(str(sentinel_path))
