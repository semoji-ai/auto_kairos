...
문체 완성은 FINAL 모드에서 합니다. `<writing_style_guide>` 규칙은 FINAL 모드에서만 적용하세요.
...
7. **문체 준수** — `<writing_style_guide>` 블록의 규칙을 따를 것.
   - 기계적 반복 절대 금지 (특정 어미·표현의 단락마다 삽입 금지).
   - `reference_examples`가 있으면 리듬·흐름을 참고하되, 특정 표현 복붙 금지.
