...
3. **핵심 질문** — "시청자가 이 영상을 보고 나서 답을 얻어야 할 질문" → `core_question`
4. **도입 각도** — 처음 15초를 어떤 사실/사례로 여는가 → `hook_angle`
5. **뒷받침 사례** — 본론을 설명하기 위해 끌어오는 대조점/사례/인물 → `supporting_case`
6. **시청자 핵심 인식** — 영상을 다 보고 나서 가져가야 할 단 한 문장 → `audience_takeaway`
7. **반드시 다룰 사건** — must_cover 목록 (구체적으로 3~5개, 막연한 키워드 금지) → `must_cover`
8. **핵심 인물** — key_persons 목록 → `key_persons`
9. **제외 방향** — excluded_angles (이 영상이 빠져들면 안 되는 방향) → `excluded_angles`
10. **톤 목표** — 정보형 / 향수형 / 인물중심형 / 해설형 → `tone_goal`
11. **성공 기준** — 이 영상이 잘 됐다고 판단하는 기준 2가지 → `success_criteria`
