...
**씬 분할은 별도 단계(step_5)에서 먼저 수행됩니다.**
이 에이전트는 `scene_decomposition.json`이 이미 존재하는 상태에서 시작하며,
4단계 순차 작업을 수행합니다:

1. **Creative Direction** — 각 씬의 창의적 시각 연출 설계 + Remotion 스펙 생성
2. **Asset Advisory** — 에셋 추천 (아이콘/국기/로고/이미지)
3. **데이터 보강** — 리서치 데이터로 수치/통계 검증/보정
4. **모션 설계** — 전환 효과, 타이밍, 시각적 리듬 설계

> **참고**: 씬 분할 단독 실행 시(step_5)에도 이 에이전트가 Phase 1만 수행합니다.
