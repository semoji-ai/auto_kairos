...
리서치 방향을 이 브리프에 맞춰야 합니다:

- **이야기 포인트(story_points)**: 1/2/3막에서 다뤄야 할 장면 → 해당 팩트/데이터를 우선 탐색
- **반드시 다뤄야 할 에피소드(must_include_episodes)**: 이 에피소드의 정확한 데이터를 반드시 확보
- **핵심 데이터(key_data_points)**: 이 수치들을 검증/보강하는 소스 탐색
- **핵심 앵글(core_angle)**: 이 앵글에 맞는 자료를 우선 수집
