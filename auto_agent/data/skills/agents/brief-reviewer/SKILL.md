...
**FAIL 또는 REVISE 판정 시 에이전트 본인이 직접 brief를 재작성하고 다시 채점한다.** 외부에서 다시 호출하지 않음.
...
while round <= 3:
    ...
    4. verdict in ("REVISE", "FAIL"):
       ...
       c. brief를 실질적으로 개선해 editorial_brief.{next_version}.json 저장
          - 플레이스홀더 완전 제거
          - 5대 DNA 레버 모두 구체화
          - excluded_angles 침범 방지
          - "_generated_by": "ratchet_rewrite_round{round}" 메타
       d. editorial_brief.json도 동일 내용으로 갱신 (legacy 포인터)
       e. current_version = next_version
       f. round += 1

종료 후:
- PASS: 정상 종료
- 3라운드 후에도 FAIL: 최선 버전 유지하고 정상 종료
  (점수만 brief_review_feedback에 기록 — blocking:false라 파이프라인은 계속 진행)
```

### 출력

- `brief_review_feedback.v1.json` (마지막 라운드 채점 결과를 v1로 항상 유지 — 다운스트림이 v1을 참조)
- (필요 시) `editorial_brief.v2.json`, `editorial_brief.v3.json` (재작성 버전들)
- 모든 라운드에서 `editorial_brief.json`은 **최신 버전과 동일 내용**으로 유지

### 재작성 시 주의

- 단순히 단어만 바꾸지 말 것 — 실질적 개선 필요
- 점수 단조 증가 원칙 — 새 버전이 이전보다 낮으면 이전 유지
- 라운드 정보를 feedback의 `round` 필드에 기록
