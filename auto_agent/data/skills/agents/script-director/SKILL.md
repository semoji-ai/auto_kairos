...
- **이야기 포인트(story_points)**: 1/2/3막 구조를 이대로 따르세요. 각 막의 핵심 장면을 씬으로 구현
- **반드시 다뤄야 할 에피소드(must_include_episodes)**: 이 에피소드를 빠뜨리면 안 됩니다. 씬으로 반드시 구현
- **핵심 앵글(core_angle)**: 전체 원고의 방향성. 이 앵글에서 벗어나면 안 됨
- **톤(tone)**: 원고의 감정 톤 결정
- **추천 구성/길이**: 씬 수와 분량 가이드
