...
6. **챕터 + 씬 + 캐릭터 마커 삽입** — prose를 쓰면서 아래 세 가지를 함께 표시합니다.
   ...
   **② 씬 경계** — `---` (**기본은 한 문장에 하나**)
   ...
   정본은 `docs/rules/scene-splitting-rules.md` 다. 요약하면 이렇다.

   **한 씬은 한 화면이다.** 문장이 바뀌면 화면도 바뀐다 — **붙이는 것이
   예외다.** 한 화면에 서너 문장을 묶으면 화면이 말을 따라가지 못한다.
   ...
   **붙여도 두 문장까지. 셋은 안 된다. 붙인 결과가 60자를 넘으면 붙이지 않는다.**

   수치를 대는 문장 · 이유를 대는 문장 · 결과를 말하는 문장은 **각각 다른
   화면**이다. 그 밖에 때·곳·주체가 바뀌거나, 인용이 나오거나(따옴표는 혼자
   둔다), 핵심 한 방일 때 가른다.

   **반전 접속사는 독립 씬으로 뗀다.** 「그런데」·「하지만」이 흐름을 뒤집는
   자리에 오면 그 한 마디만 씬으로 세운다 — 흰 화면에 글자만 띄우고 1.2초
   만에 넘어간다. 말에 묻어 두면 그냥 지나간다. 편당 3~7회.
   접속사 뒤에 조사가 붙으면(「그때까지는」) 접속사가 아니므로 떼지 않는다.

   - 목표: **평균 30자 안팎.** 60자 넘는 씬과 두 문장 이상인 씬은 한 자릿수
   - 12분 분량이면 `---` 가 **150~180개**가 정상이다
     (예전 기준인 「8분에 40~50개」는 한 씬에 서너 문장이 묶이던 때의 수치다)
   ...
   **④ 자막 마커** — `<!-- caption: 항목1 / 항목2 -->` (내레이션에서 뺀 용어·수치를 화면에만 표시)
   ...
   - **chapters 모드 처리 규칙**: caption 마커의 각 항목을 해당 씬의 `items` 배열에 넣는다.
     항목이 1개이고 씬에 headline이 비어 있으면 `headline`에 넣어도 된다.
     narration에는 절대 포함하지 않는다 (이미 빠진 텍스트다).
   - 예시:
     ```
...
3. **합칠 블록을 찾으세요.** 같은 장면의 연속(예: "가족의 다툼." / "서로 다른 변호사." /
   ...
   홀로 선 접속어 블록("그런데", "대신")은 **아래 지움 실험으로 판정**합니다.
4. **`infoStructure`를 채우세요.** 위 표의 신호를 보고 정합니다.
5. **마지막에 `statement` 개수를 세세요.** 20%를 넘으면 다시 보고 줄이세요.

#### 홀로 선 접속어 — 지움 실험

「그런데」처럼 접속어 하나가 한 블록인 자리가 있습니다. **무조건 합치면 안 됩니다.**
뜸을 들이는 것이 연출인 자리가 있고, 빈 컷만 늘리는 자리가 있습니다.

**뒤따르는 문장이 제 일을 하고 있는지 보세요. 표면 어휘로 판정하지 마세요.**

> **지움 실험 — 접속어 바로 뒤 문장을 지워 본다.**
> 지워도 그다음 문장이 성립하면 → 그 문장은 제 화면을 가질 자격이 없다. **합친다.**
> 지우면 그다음 문장이 성립하지 않으면 → 그 문장이 그릇을 세우고 있다. **접속어를 독립 컷으로 둔다.**

합치기로 정했다면 **접속어 블록이 아니라 그 다음 블록에 `mergeWithPrev: true`** 를 찍습니다.
그래야 접속어가 뒤 문장을 끌고 한 씬이 됩니다(앞 씬에 딸려 가지 않습니다).

실제 사례 — 세모지 디아지오편.

| 자리 | 뒤 문장 | 지우면 | 판정 |
|---|---|---|---|
| 「그런데」 | 그들을 몰아낸 장본인인 스태퍼드 후작이, | — (반전 내용 자체) | **독립 컷** |
| 「그런데,」 | 이 실패가 알려주는 것이 있습니다. | 「~다는 것.」이 받을 그릇이 사라짐 | **독립 컷** |
| 「그런데」 | 증류소를 다시 지을 때 한 일이 **흥미롭습니다**. | 뒤 문장 멀쩡함 | **합친다** |

셋째 자리를 독립 컷으로 두면 **정보 없는 컷이 두 번 연속**됩니다 — 「그런데」(1초, 정보 0)
+ 「흥미롭습니다」(2초, 정보 0). 뜸은 한 번만 들입니다.

> ⚠️ 「알려주는 것이 있습니다」와 「흥미롭습니다」는 겉이 닮았지만 반대입니다.
> 앞엣것은 「~것이 있습니다 → ~다는 것」으로 **호응하는 짝**이 있어 감췄다가 알려주는
> 구조를 세웁니다. 뒤엣것은 짝 없는 **감상**입니다. 어미만 보고 묶지 마세요.
...
6. headline + source 작성
   ...
   **짧은 블록(1~20자) 처리법 — 이 순서로 검토하세요:**
   ...
   | 장면 전환 접속어 단독 (예: "그런데", "대신", "그리고") | **독립 씬으로 만들지 말고** 다음 블록에 붙여 한 씬으로 |
