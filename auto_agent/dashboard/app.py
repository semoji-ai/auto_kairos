...
async def storyboard_scene_by_slug(request: Request, slug: str, scene_num: int):
    pm = get_pm()
    project = pm.get_project(slug=slug)
    if not project:
        return HTMLResponse("Not found", status_code=404)
    ...
    return templates.TemplateResponse("partials/_storyboard_scene.html", {
        "request": request, "scene": scene, "subtitles": scene_subs,
        "slug": slug, "project_id": project["id"],
    })


@app.get("/api/p/{slug}/editor/scenes/{scene_num}")
async def editor_scene_by_slug(slug: str, scene_num: int):
    """씬 에디터용 JSON 데이터."""
    pm = get_pm()
    project = pm.get_project(slug=slug)
    if not project:
...
@app.get("/api/p/{slug}/manifest-meta")
async def manifest_meta_by_slug(slug: str):
    """매니페스트 메타 정보."""
    pm = get_pm()
    project = pm.get_project(slug=slug)
    if not project:
    ...
    config = pm.get_config(project["id"]) if hasattr(pm, "get_config") else {}
    meta = {
        ...
        "vizFont": "Pretendard",
        "videoTheme": config.get("video_theme", "dark") if config else "dark",
        "artStyle": config.get("art_style", "") if config else "",
    }
    return {"meta": meta}


@app.get("/api/p/{slug}/scene/{scene_num}/image-candidates")
async def image_candidates_by_slug(slug: str, scene_num: int):
...
async def select_image_by_slug(request: Request, slug: str, scene_num: int):
    import shutil
    pm = get_pm()
    project = pm.get_project(slug=slug)
    if not project:
