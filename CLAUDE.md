...
### 세션 요약 품질 기준 (핵심만 정확하게)

**프론트매터 (검색 정확도 결정):**
- title: 키워드 풍부하게 (프로젝트명 + 핵심 결과 포함)
- tags: 프로젝트명, 핵심 기술, 수치 결과
- projects: [slug-1, slug-2] (프로젝트별 필터)
- keywords: [수치 포함 — "래칫 85점", "$2.51", "75씬"]

**본문 — 5가지만 기록 (나머지는 git에 있으니 불필요):**
1. **설계 결정**: 왜 이 방식을 선택했는가 + 근거 (코드 변경은 git, 이유만 기록)
2. **실험 결과**: 수치로 (래칫 81→85, 에이전트 395초, $2.51)
3. **미해결 문제**: 구체적으로 (뭐가 안 되고, 원인 추정은 뭔지)
4. **사용자 피드백**: 방향 전환 지시 ("트래픽 vs 차별화 균형", "브랜드백과 70%")
5. **다음 우선순위**: 번호 매겨서

6. **시행착오**: 실패한 접근 + 왜 실패 + 최종 해결 (한 줄 압축)
   예: "Write로 68씬 수정 시도 → 에이전트가 토큰 부담으로 스킵 → Edit으로 전환해서 해결"
   예: "launchd load/unload → 스케줄 미작동 → bootout/bootstrap으로 해결"

**기록하지 않는 것:**
- 코드 수정 내역 (git commit에 있음)
- 중간 디버깅 로그 (결론만)
- 도구 사용법 (CLAUDE.md/SKILL.md에 있음)
