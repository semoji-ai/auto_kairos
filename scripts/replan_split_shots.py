...
**`continuous` — 같은 순간, 같은 자리**
  카메라만 움직인 것입니다. 그러니 **바뀌면 안 되는 것**이 많습니다.
  · 같은 인물이 같은 옷, 같은 자세, 같은 자리에 있어야 합니다
  · 빛의 방향과 색이 같아야 합니다 (아침이었다가 저녁이 되면 안 됩니다)
  · 소품이 있던 자리에 그대로 있어야 합니다
  · 사람이 보는 방향(왼쪽/오른쪽)이 뒤집히면 안 됩니다
  · 크기는 **한 단계 이상** 확실히 바꿉니다. 조금만 바꾸면 튀어 보입니다
    (wide → close 는 좋고, medium → medium 은 안 됩니다)
