...
def build(scene: dict) -> str:
    ...
    lines = [
        ...
        "빛은 색면의 밝기 차로만 나타낸다",
        "",
        # 글자를 통째로 막던 규칙이었다. gpt-image-2 가 한글을 못 쓴다고 보고
        # 「모든 면을 빈 색면으로」 못 박았는데, **틀린 전제였다.**
        # 도해를 한 장으로 그려 보니 「사농공상 / 장손이 맨 아래 칸으로」도,
        # 「구인회상회 자본금과 견주면」도 한 글자도 안 틀렸다.
        #
        # 그 규칙이 실제로 화면을 망가뜨리고 있었다. 씬1063 은 프롬프트에
        # 「간판 판면은 매끈한 빈 색면이다」라고 적혀 있어, 「구인회가 세운
        # 락희화학공업사였죠」라는 결정적 한 줄이 백지 간판으로 나왔다.
        # 씬968 의 교지도 백지라 「구교리댁」이 화면에 없었다.
        #
        # 그래서 가른다 — **그 글자가 곧 뜻인 자리에만** 넣는다.
        # 틀리게 나오면 레이어를 갈라 애프터이펙트에서 바꾼다.
        # 씬이 넣을 글자를 정해 두었으면 그것만 넣는다. 모델이 알아서 쓰게 두면
        # 회사 이름을 지어내거나 엉뚱한 말을 적는다.
        (f"Text-in-image: {scene['text_in_image']}. 여기 적은 글자만 넣고, "
         "그 밖의 면 — 지나가는 거리의 간판, 책 표지, 상자, 벽보 — 은 매끈한 빈 "
         "색면으로 남겨 둡니다. 한자와 일본어 가나 대신 한글로 씁니다"
         ) if (scene.get("text_in_image") or "").strip() else
        "Text-in-image: 간판·현판·표지처럼 **그 글자가 곧 그 장면의 뜻인 자리**에는 "
        "획이 또렷한 한글을 씁니다. 회사 이름이 붙는 순간, 집 이름으로 불리는 장면, "
        "액수가 곧 사건인 장면이 그렇습니다. "
        "그 밖의 면 — 지나가는 거리의 간판, 책 표지, 상자, 벽보 — 은 매끈한 빈 색면으로 "
        "남겨 두어 화면이 어수선해지지 않게 합니다. 넣는 글자는 꼭 필요한 것만 "
        "짧게 쓰고, 한자와 일본어 가나 대신 한글로 씁니다",
        "",
