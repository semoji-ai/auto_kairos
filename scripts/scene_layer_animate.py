...
def build_background_prompt(scene: dict, art_style: dict) -> str:
    parts = [
        ...
        "",
        "**ATTACHED REFERENCE (STYLE ONLY):**",
        "- The attached image is the ART STYLE reference — match its drawing technique, colors, and proportions exactly.",
        "- Do NOT copy or include the person/character from the reference. Use it for the drawing style ONLY.",
        "",
...
def build_sprite_prompt(character: dict, art_style: dict) -> str:
    ...
    parts = [
        ...
        f"한국어 묘사: {desc_ko}",
        "",
        "**ATTACHED REFERENCES (label by order):**",
        "- 1번(첫 번째) attached image = ART STYLE reference: match its drawing technique, colors, and body proportions exactly — do NOT copy its character identity.",
        "- 2번(있으면) attached image = THIS character's sheet: match that person's face/hair/outfit/proportions exactly. Same person, do NOT redesign.",
        "- Do NOT re-specify or alter proportions beyond what the references show — the references define proportion.",
        "",
