...
## 원고 편집 규칙 (LG 13부작에서 반복 사고 후 확립)

- **패치 편집 후 반드시 `scripts/check_manuscript.py`를 돌린다.** 평가에 넘기기 전, 녹음에 넘기기 전 두 번.
  ```bash
  .venv/bin/python scripts/check_manuscript.py --dir <원고폴더>
  ```
  근접 중복(90%+ 유사), 챕터 번호 불연속, 마커 문법, 미닫힘 주석을 검출한다.

- **문자열 교체 편집은 원본을 남긴다.** 교체문을 붙이고 원본을 안 지워 같은 내용이 두 번 나오는 사고가 4편·8편에서 반복됐다. 교체는 반드시 `old → new` 쌍으로 하고, `assert old in text`로 매칭을 확인한 뒤 `replace(old, new, 1)`을 쓴다. 추가(append)로 수정하지 말 것.

- **챕터 제목 줄이 유실될 수 있다.** EP04에서 `# Ch1.` 헤딩이 패치 중 사라져 본문만 남았다. 검사기가 잡았다. 챕터를 삽입·재번호할 때는 전체 헤딩 목록을 다시 출력해 확인할 것.

- **마커 표기는 `caption:`과 `chars:` 두 가지뿐.** `subtitle:` 등 변형 금지 (EP13에서 발견).

## 3인 페르소나 채점 운영

- 평가 프롬프트에 **"직전 지시 반영 판정표"를 요구**하면 이미 고친 항목을 재지적하지 않아 라운드가 빨리 수렴한다.
- 채점자가 낮은 점수를 주는 원인은 거의 항상 **① research 근거를 넘어선 인과 단정 ② 간접 서술("~했다고 전해집니다")을 직접 대사로 만든 것** 둘 중 하나다. 집필 시 이 둘을 먼저 자기검열할 것.
- 분량이 모자라면 집필로 늘리지 말고 **타깃 리서치를 먼저 발주**한다. 근거 없이 늘리면 사실 정확성 점수가 떨어진다. (9편에서 블랙라벨 리서치 발주 → 5.7분→8.2분, 8.9→9.5)
- **codex가 산출 파일을 NAS 심링크 경로에 쓰는 경우가 있다.** 프롬프트에 출력 절대경로를 명시하고, 결과가 안 보이면 `/Volumes/...` 쪽도 확인할 것.
